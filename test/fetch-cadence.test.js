...
function holder(db, { seenDaysAgo, ticker = 'AAA' }) {
  ...
  if (seenDaysAgo !== null) {
    db.prepare('UPDATE users SET last_seen_at = ? WHERE id = ?').run(daysAgo(seenDaysAgo), u);
  }
...
test('an alert keeps a ticker daily even when every holder is dormant', () => {
  ...
  assert.strictEqual(tickerTier(db, 'AAA', NOW), 'cold', 'precondition: dormant');
...
test('the algorithm alert alone does not make everything hot', () => {
  ...
  const db = migratedDb();
  const u = holder(db, { seenDaysAgo: 400 });
  assert.strictEqual(db.prepare('SELECT algo_alerts_enabled AS e FROM users WHERE id = ?').get(u).e, 1,
