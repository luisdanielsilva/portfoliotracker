...
**And the hero above them was still inventing its numbers — 2026-09-26, issue #15.** The prose said
ten at &euro;100 and ten more at **&euro;70** for an average of **&euro;85**; the chart a few hundred
pixels below it drew ten more at **&euro;68.63** for an average of **&euro;84**. Two sets of figures
for one worked example, close enough that nobody would notice &mdash; which is the problem on a page
arguing that a number should be current and checkable rather than roughly right. The hero's lede and
the four-row panel beside it are generated by `landing-figures.js` from the same series as the
figures, behind `figure:hero-lede` and `figure:hero-stats` markers, so they cannot drift again.

**Each panel row also says what the rule did, not only what it is.** `+75% on cost` now adds that on
AMD it emailed at &euro;154 and the stock ran on to &euro;182 before turning; `&minus;20% off its
high` adds that it caught that break at &euro;146 with the window ending at &euro;128. The settled
hero message is untouched &mdash; both directions, judged against your own average cost, and the "it
won't call the bottom or the top" line stays exactly where it was.
